...
@Component({
  selector: 'page-staffchat',
  templateUrl: 'staffchat.html',
  queries: {
        ...
        messages.forEach((message) => {
          ...
          if (message.SenderId == val) {
            if (message.RecipientId == that.item.Id) {
              this.htmlchs += '<div class="chatBubble"><div class="chat-bubble rightcol" style="float:right"><div class="message">' + message.Message + '</div><div class="message-detail"><p style="font-weight:bold;">' + username + '</p><p>' + Date[0] + ':' + time[0] + '</p></div></div></div>';
            }
          } else if (message.SenderId == that.item.Id) {
            if (message.RecipientId == val) {
              this.htmlchs += '<div class="chatBubble"><div class="chat-bubble leftcol" style="float:left"><div class="message">' + message.Message + '</div><div class="message-detail"><p style="font-weight:bold;">' + recivername + '</p><p>' + Date[0] + ':' + time[0] + '</p></div></div></div>';
            }
          } else {}
        });
        window['jQuery']('#checksms').html(this.htmlchs);
